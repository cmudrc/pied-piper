...
def create_model(path, population, household_size):
    ...
    model.bake()
    model.society.generate(num=population, gini_index=0.4)
    return model
...
for population in populations:
    for household_size in household_sizes:
        model = create_model(
            path=path, population=population, household_size=household_size
        )
        models.append(model)
...
def run(model):
    ...
    """
    model.run(n=100, step_size=3600 * 2, save=True, save_transactions=True, report=True)
