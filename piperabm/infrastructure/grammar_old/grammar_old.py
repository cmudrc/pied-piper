...
class InfrastructureGrammar:
    
    def apply_infrastructure_grammars(self, report=True):
        """
        ...
        """
        grammars = [
            self.infrastructure_grammar_rule_1,
            self.infrastructure_grammar_rule_2,
            self.infrastructure_grammar_rule_3,
            self.infrastructure_grammar_rule_4,
        ]
    ...
    def infrastructure_grammar_rule_2(self):
        ...
        ''' loop for the node '''
        for node_index in self.all_environment_nodes:
            if node_index in self.all_environment_nodes: # to make sure it is not deleted in previous runs
                node_item = self.get(node_index)

                ''' loop for the edge '''
                for edge_index in self.all_environment_edges:
                    if edge_index in self.all_environment_edges and node_index in self.all_environment_nodes: # to make sure it is not deleted in previous runs
    ...
    def infrastructure_grammar_rule_3(self):
        ...
        for edge_index in self.all_environment_edges:
            if edge_index in self.all_environment_edges: # to make sure it is not deleted in previous runs
                edge_item = self.get(edge_index)

                ''' loop for the second edge '''
                for other_edge_index in self.all_environment_edges:
                    if edge_index != other_edge_index:
                        if other_edge_index in self.all_environment_edges and edge_index in self.all_environment_edges: # to make sure it is not deleted in previous runs
