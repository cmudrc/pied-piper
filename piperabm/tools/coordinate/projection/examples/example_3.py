...
from piperabm.tools.coordinate.projection.latlong_xy import latlong_to_xy
...
latitudes_linspace = list(np.linspace(latitude_0 - DELTA/2, latitude_0 + DELTA/2, num))
longitudes_linspace = list(np.linspace(longitude_0 - DELTA/2, longitude_0 + DELTA/2, num))
...
for i in range(num ** 2):
    x, y = latlong_to_xy(latitude_0, longitude_0, latitude=latitudes[i], longitude=longitudes[i])
    xs.append(x)
    ys.append(y)
